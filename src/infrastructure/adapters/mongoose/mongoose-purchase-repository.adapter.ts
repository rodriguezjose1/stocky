...
@Injectable()
export class MongoosePurchaseRepositoryAdapter implements PurchaseRepositoryPort {
  private purchaseModel = Model<any>;
  constructor(@InjectConnection() private connection: Connection) {
    this.purchaseModel = this.connection.model(PurchaseModel.name, PurchaseSchema);
  }
  ...
  private mapToDomain(purchaseModel: PurchaseModel): Purchase {
    return new Purchase(
      ...
      purchaseModel.date,
      purchaseModel.details.map((detail) => new PurchaseDetail(detail.product_id.toString(), detail.quantity, detail.unit_price)),
    );
  }
}
